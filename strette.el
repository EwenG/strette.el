...
(defun strette-alist-set (alist k v)
  "Non destructively associate the value V with- the key K in the alist ALIST. If ALIST already contains K, then its value (the first one if there are multiple K) is changed to V. If ALIST does not contain a key K, they the pair (K . V) is cons-ed to ALIST. Keys are compared using `equal`"
  (cond ((null alist) `((,k . ,v)))
